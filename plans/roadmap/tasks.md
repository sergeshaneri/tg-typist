...
| ID | Depends | Status | Goal | Likely files | Checks | Risks |
|---|---|---:|---|---|---|---|
| S1.1 | H0.1 | BLOCKED | Create Python project scaffold with package layout and dependency config. | `pyproject.toml`, `src/tg_typist/__init__.py`, `.gitignore`, `.env.example` | `python -m compileall -q src` | Wrong dependency versions or missing Railway-compatible start path. |
